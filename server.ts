...
app.post("/api/extract", async (req, res) => {
  try {
    ...
    let response;
    const isSvgMime = mimeType && (mimeType.toLowerCase().includes("svg") || mimeType.toLowerCase().includes("xml"));

    if (isSvgMime) {
      // SVG code is textual XML. We decode it from base64 and feed it as raw text reference.
      // This bypasses Gemini's standard image/svg+xml MIME restriction beautifully.
      let svgText = "";
      try {
        svgText = Buffer.from(image, "base64").toString("utf-8");
      } catch (e) {
        svgText = "";
      }

      writeLog("Processing SVG representation safely via semantic XML code matching...");
      response = await genAI.models.generateContent({
        model: "gemini-3.5-flash",
        contents: `You are given the SVG XML code of an animated character. 
Analyze and inspect this SVG XML code (looking at tags, geometries, coordinates, shapes, group labels, and hex colors) to discover the identity of this character.

Reply ONLY with the exact short name of the character (e.g. 'Pikachu', 'Doraemon', 'Totoro', 'Hello Kitty'). If you can't discover the specific name, use a highly precise descriptive 2-3 word name (e.g. 'purple baby bear', 'chubby cute mouse').

DO NOT use punctuation, sentences, markdown formatting, or explanations. Reply with just the name itself.

SVG XML content:
${svgText}`
      });
    } else {
      // Standard Multimodal input for PNG, JPEG, WebP, etc.
      const imagePart = {
        inlineData: {
          mimeType,
          data: image,
      ...
      writeLog("Requesting standard multimodal analysis from Gemini...");
      response = await genAI.models.generateContent({
        model: "gemini-3.5-flash",
    ...
    writeLog(`Resolution system redirected to offline backup flow: ${err?.message || err}.`);
