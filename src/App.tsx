...
export default function App() {
  ...
  const getDisplayPrompt = (): string => {
    ...
    const activeName = manualCharacterName || characterName || "";
    return compilePromptLocally(activeName);
  };

  const copyToClipboard = () => {
    const textToCopy = getDisplayPrompt();
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 mt-4 relative z-10 flex flex-col gap-8">
        ...
        <section className="text-center sm:text-left flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-zinc-200/80 gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 flex items-center justify-center sm:justify-start gap-2.5">
              <Layers className="w-7 h-7 text-indigo-600 stroke-[2.2]" /> Chế Tạo Đồ Chơi In 3D
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <section className="lg:col-span-7 flex flex-col gap-6">
            ...
            {/* Loading Mask Structure & Results Container */}
              ...
              {isLoaderActive ? (
                ...
                >
                  <div className="bg-white rounded-2xl border border-zinc-200 p-5 shadow-sm relative">
                    ...
                    <div className="flex items-center justify-between pb-3.5 border-b border-zinc-100 mb-4 gap-2 flex-wrap sm:flex-nowrap">
                      <div className="flex items-center gap-2">
                        <div className={`h-2.5 w-2.5 rounded-full ${generatedPrompt ? "bg-emerald-500 animate-pulse" : "bg-indigo-500 animate-pulse"}`} />
                        <span className="text-xs font-semibold text-slate-500 uppercase tracking-widest font-mono">Bản Prompt Đầu Ra</span>
                      </div>
                      
                      {/* Identified / Status Badge card */}
                      {characterName ? (
                        <div className="px-2.5 py-1 bg-emerald-50 border border-emerald-200 text-[11px] font-mono rounded-lg text-emerald-800 flex items-center gap-1.5 shadow-2xs">
                          <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                          Nhân vật: <strong className="text-indigo-600 font-bold font-sans">{characterName}</strong>
                        </div>
                      ) : (
                        <div className="px-2.5 py-1 bg-amber-50 border border-amber-200 text-[11px] font-mono rounded-lg text-amber-800 flex items-center gap-1.5 shadow-2xs">
                          <Layers className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                          Chế độ: <strong>Xem trước bản phác đoán</strong>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="group relative">
                      ...
                      <button
                        ...
                        type="button"
                        className={`absolute top-3 right-3 p-2.5 rounded-xl border transition-all duration-200 flex items-center gap-1.5 text-xs font-semibold shadow-xs hover:scale-103 cursor-pointer z-10
                          ${copied 
                    ...
                    {/* Instruction footer */}
                    <div className="mt-4 p-3 bg-zinc-50 border border-zinc-200 rounded-xl flex items-start gap-2.5 text-xs text-slate-500">
                      <Info className="w-4 h-4 text-indigo-500 shrink-0 mt-0.5" />
                      <div>
                        {characterName ? (
                          <span>Prompt đã hoàn thiện tự động tích hợp nhân vật <strong className="text-slate-700">{characterName}</strong> với phong cách đồ chơi dạng chunky vinyl đặc trưng.</span>
                        ) : (
                          <span>Đây là một bản prompt cấu trúc gốc. Bạn có thể sao chép nó ngay lập tức, điền tên nhân vật mong muốn, hoặc tải ảnh lên để AI tự động trích xuất thông minh!</span>
                        )}
                      </div>
                    </div>
